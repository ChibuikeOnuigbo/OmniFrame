...
export interface VoiceIsolationOptions {
  ...
  strength?: number // 0.0 to 1.0 (default 0.9)
...
export async function processVoiceIsolation(
  ...
  try {
    ...
    onProgress?.(55, options.mode === 'keep_vocal' ? 'Isolating center vocal formants…' : 'Canceling center vocal and preserving bass…')
...
export async function executeVoiceIsolationForClip(
  ...
  const labelPrefix = options.mode === 'keep_vocal' ? '[Vocal Isolated]' : '[Vocal Removed]'
  const baseName = asset.name.replace(/\.[^/.]+$/, '')
  const newAssetName = `${labelPrefix} ${baseName}.wav`

  const result = await processVoiceIsolation(asset.url, options, onProgress)
