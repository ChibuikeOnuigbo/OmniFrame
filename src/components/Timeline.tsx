...
export function Timeline() {
  ...
  // Active clip drag listeners
  const startClipDrag = (clip: Clip, e: React.PointerEvent) => {
    const lanes = lanesRef.current
    if (!lanes) return
    const target = evaluateDragTarget(e.clientY, lanes, tracks, clip.kind)
    ...
    const onPointerMove = (moveEv: PointerEvent) => {
      ...
      const d = (moveEv.clientX - dragRef.current.startX) / px
      const candidateTime = Math.max(0, snapTime(dragRef.current.origStart + d))
