...
function TopBar() {
  ...
  const menuItems: Record<string, Array<{ label: string; shortcut?: string; action: () => void; disabled?: boolean }>> = {
    ...
    Help: [
      ...
      { label: 'Local processing boundary', action: () => state.flash('Import, preview and browser export stay local; native FFmpeg is explicit.') },
    ],
  };
...
function LeftRail() {
  ...
  const importFile = async (file: File) => {
    const kind = file.type.startsWith('audio/') ? 'audio' : file.type.startsWith('image/') ? 'image' : file.type.includes('gltf') || file.name.toLowerCase().endsWith('.glb') || file.name.toLowerCase().endsWith('.gltf') ? 'model' : 'video';
...
async function runTrackingJob(state: EditorState, maskTracking: boolean): Promise<void> {
  const clip = selectedClip(state);
  const asset = state.project.assets.find((item) => item.id === clip?.assetId);
  if (!clip || !asset || asset.kind !== 'video' || asset.missing || !asset.sourcePath || asset.sourcePath.startsWith('demo:')) return state.flash('Relink a local video and select its clip before analyzing.');
...
async function runOmniframe(state: EditorState): Promise<void> {
  const clip = selectedClip(state);
  const asset = state.project.assets.find((item) => item.id === clip?.assetId);
  if (!clip || !asset || asset.kind !== 'video' || asset.missing || !asset.sourcePath || asset.sourcePath.startsWith('demo:')) return state.flash('Relink a local video and select its clip before applying Omniframe.');
...
function MediaBin() {
  ...
  const importFile = async (file: File) => {
    const kind = file.type.startsWith('audio/') ? 'audio' : file.type.startsWith('image/') ? 'image' : file.name.toLowerCase().endsWith('.glb') || file.name.toLowerCase().endsWith('.gltf') ? 'model' : 'video';
  ...
  return <div className="modal-scrim" onClick={() => state.set({ showMediaBin: false })}><div className="media-dialog" onClick={(event) => event.stopPropagation()}><div className="dialog-head"><div><span className="section-kicker">MEDIA BIN</span><h2>Local sources.</h2></div><button className="icon-button" onClick={() => state.set({ showMediaBin: false })}><X size={16} /></button></div><div className="media-actions"><button className="primary-button" onClick={() => input.current?.click()}><Upload size={15} /> Import media</button><button className="outline-button" onClick={state.restoreAutosave}><RotateCcw size={14} /> Restore recovery</button><input ref={input} hidden type="file" accept="video/*,audio/*,image/*,.glb,.gltf" onChange={(event) => { const file = event.target.files?.[0]; if (file) void importFile(file); event.currentTarget.value = ''; }} /><input ref={relinkInput} hidden type="file" accept="video/*,audio/*,image/*,.glb,.gltf" onChange={(event) => { const file = event.target.files?.[0]; if (file) void relinkFile(file); event.currentTarget.value = ''; }} /></div><div className="media-list">{assets.length === 0 ? <div className="media-empty"><FilePlus2 size={28} /><strong>No imported media.</strong><span>Nothing is uploaded automatically. Choose a local video, audio file, image or GLB.</span></div> : assets.map((asset) => <div key={asset.id} className="media-item" role="button" tabIndex={0} onClick={() => selectAsset(asset)} onKeyDown={(event) => { if (event.key === 'Enter') selectAsset(asset); }}><span className={`media-type ${asset.kind}`}>{asset.kind === 'model' ? <Box size={16} /> : asset.kind === 'audio' ? <AudioLines size={16} /> : asset.kind === 'image' ? <Aperture size={16} /> : <Film size={16} />}</span><span className="media-meta"><strong>{asset.name}</strong><small>{asset.kind} · {asset.missing ? 'missing — relink required' : asset.width ? `${asset.width}×${asset.height}` : 'metadata pending'}{asset.duration ? ` · ${asset.duration.toFixed(2)}s` : ''}</small></span><span className={`media-source ${asset.missing ? 'missing' : ''}`}><Link2 size={12} /> {asset.missing ? 'missing' : asset.proxyId ? 'proxy ready' : 'local'}</span>{asset.missing && <button className="outline-button compact" onClick={(event) => { event.stopPropagation(); setRelinkId(asset.id); relinkInput.current?.click(); }}>Relink</button>}{!asset.missing && !asset.proxyId && <button className="icon-button" title="Proxy status" onClick={(event) => { event.stopPropagation(); state.flash('Proxy generation requires the native FFmpeg worker; the browser will not pretend a proxy exists.'); }}><Gauge size={14} /></button>}</div>)}</div><p className="dialog-note">Browser object URLs remain local to this session. Recovery keeps missing assets relinkable; native FFmpeg is required before a proxy can be generated.</p></div></div>;
}
