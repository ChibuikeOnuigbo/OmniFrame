...
function useKeyboardShortcuts() {
  useEffect(() => {
    const onKey = (event: KeyboardEvent) => {
      ...
      if ((event.metaKey || event.ctrlKey) && event.key.toLowerCase() === 's') { event.preventDefault(); if (isDesktopShell()) state.saveProject(); return; }
...
function Landing() {
  ...
  return <main className="landing-page">
    ...
    <header className="landing-nav content-width"><Logo /><div className="landing-nav-actions"><button className="primary-button compact" onClick={openEditor}>Open editor <ArrowDownToLine size={13} /></button></div></header>
    <section className="landing-hero content-width"><div className="hero-copy"><h1>Edit the moment.<br /><em>Keep the frame.</em></h1><p className="hero-subtitle">Cut, mask, track and export.</p><div className="hero-actions"><button className="primary-button" onClick={openEditor}>Open OmniFrame <Play size={15} fill="currentColor" /></button></div><div className="hero-meta"><span><Lock size={13} /> Local-first workflow</span><span><Check size={13} /> Non-destructive tools</span></div></div><LandingWorkbench /></section>
...
function LandingWorkbench() {
  return <div className="hero-workbench"><div className="hero-window-bar"><div className="window-lights"><i /><i /><i /></div><span>omniframe / sequence 01</span><span className="window-live">local preview</span></div><div className="hero-window-body"><div className="hero-side-tools"><div className="mini-tool active"><MousePointer2 size={13} /></div><div className="mini-tool"><Brush size={13} /></div><div className="mini-tool"><Target size={13} /></div><div className="mini-tool"><Box size={13} /></div></div><div className="hero-view"><div className="hero-view-label">VIEWER / MEDIA</div><div className="hero-empty"><Film size={24} /><strong>Import a shot to begin</strong><span>Canvas preview · local sequence</span></div><div className="hero-play"><Play size={16} fill="currentColor" /></div></div></div><div className="hero-timeline"><div className="hero-timeline-ruler"><span>00:00</span><span>00:02</span><span>00:04</span><span>00:06</span></div><div className="hero-track-row"><span className="hero-track-label">V1</span><div className="hero-clip first" /><div className="hero-playhead" /></div><div className="hero-track-row slim"><span className="hero-track-label">A1</span><div className="hero-audio" /></div></div></div>;
}
...
function TopBar() {
  ...
  const menuItems: Record<string, Array<{ label: string; shortcut?: string; action: () => void; disabled?: boolean }>> = {
    File: [
      ...
      ...(desktop ? [{ label: 'Save project', shortcut: '⌘S', action: state.saveProject }] : []),
  ...
  return <header className="editor-topbar">
    ...
    <div className="topbar-actions"><span className="native-status"><span className="status-dot" /> {nativeLabel}</span><div className="topbar-history" aria-label="History"><button className="studio-text-button" disabled={!state.history.canUndo()} title={state.history.undoLabel() ?? 'Undo'} onClick={state.undo}><Undo2 size={15} /><span>Undo</span></button><button className="studio-text-button" disabled={!state.history.canRedo()} title={state.history.redoLabel() ?? 'Redo'} onClick={state.redo}><Redo2 size={15} /><span>Redo</span></button></div><button className="studio-text-button command-action" title="Command palette" onClick={() => state.set({ showCommandPalette: true })}><Command size={15} /><span>Commands</span></button><label className="aspect-control"><span>Canvas</span><select aria-label="Sequence aspect ratio" value={aspect} onChange={(event) => state.setAspectRatio(event.target.value as '16:9' | '1:1' | '9:16' | '4:5')}><option value="16:9">16:9</option><option value="1:1">1:1</option><option value="9:16">9:16</option><option value="4:5">4:5</option></select></label><label className="performance-control"><span>Preview</span><select className="performance-select" aria-label="Preview performance" value={state.performance} onChange={(event) => state.set({ performance: event.target.value as EditorState['performance'] })}><option>AUTO</option><option>QUALITY</option><option>BALANCED</option><option>PERFORMANCE</option><option>ULTRA PREVIEW</option></select></label>{desktop && <button className="outline-button compact project-action" title="Save project" onClick={state.saveProject}><Save size={14} /><span>Save project</span></button>}<button className="primary-button compact" title="Export media" onClick={() => state.set({ showExport: true })}><Download size={14} /><span>Export</span></button></div>
  </header>;
}
...
function PreviewPanel() {
  ...
  const draw = () => {
    ...
    const width = Math.max(2, Math.round((canvas.clientWidth || rect.width) * dpr));
    const height = Math.max(2, Math.round((canvas.clientHeight || rect.height) * dpr));
  ...
  const fullscreen = () => { const stage = canvasRef.current?.closest('.preview-stage'); if (stage instanceof HTMLElement && stage.requestFullscreen) void stage.requestFullscreen(); else setNote('Fullscreen is not available in this browser.'); };
  ...
  return <section className="preview-panel"><PanelToolbar title="Preview" subtitle={`${state.workspace === 'edit' ? 'Omniframe Edit' : state.workspace === 'mask' ? 'Masking mode' : state.workspace === 'maskTracking' ? 'Mask tracking' : state.workspace.toUpperCase()}`} actions={<><div className="zoom-control"><ZoomOut size={13} /><select value={state.previewZoom} onChange={(event) => state.set({ previewZoom: event.target.value })}><option value="fit">Fit</option><option value="25%">25%</option><option value="50%">50%</option><option value="100%">100%</option><option value="125%">125%</option><option value="150%">150%</option><option value="200%">200%</option><option value="300%">300%</option></select><ZoomIn size={13} /></div><button className="studio-text-button" title="Toggle guides" onClick={() => state.set({ guides: !state.guides })}><Grid2X2 size={14} /><span>{state.guides ? 'Hide guides' : 'Guides'}</span></button><button className="studio-text-button" title="Fullscreen viewer" onClick={fullscreen}><Aperture size={14} /><span>Fullscreen</span></button></>} /><div className="preview-stage" style={{ backgroundSize: `${Math.max(16, 32 * viewScale)}px ${Math.max(16, 32 * viewScale)}px`, backgroundPosition: `${pan.x}px ${pan.y}px` }} onWheel={(event) => { event.preventDefault(); changeZoom(event.deltaY > 0 ? -.1 : .1); }} onPointerDown={beginPan} onPointerMove={movePan} onPointerUp={endPan} onPointerCancel={endPan}><video ref={videoRef} className="source-media" muted playsInline preload="auto" onLoadedMetadata={() => setSourceReady(true)} onCanPlay={() => setSourceReady(true)} onTimeUpdate={onVideoTime} onError={() => setNote('The browser could not decode this video. Try generating a proxy in the Media bin.')} /><img ref={imageRef} className="source-media" alt="" onLoad={() => setSourceReady(true)} onError={() => setNote('The browser could not decode this image.')} /><div className="preview-frame" style={{ aspectRatio: `${sequence.width} / ${sequence.height}`, transform: viewTransform, transformOrigin: 'center center' }}><canvas ref={canvasRef} className="preview-canvas" /><canvas ref={overlayRef} className="preview-overlay" onPointerDown={pointer} onPointerMove={pointer} onPointerUp={finishPaint} onPointerCancel={() => state.setMaskPath([])} /></div><div className="viewer-corner viewer-corner-tl">{state.performance} <span>·</span> {Math.round(viewScale * 100)}%</div><div className="viewer-corner viewer-corner-br"><span className="status-dot" /> canvas 2D <span>·</span> active</div><div className="viewer-corner viewer-ratio-badge">Canvas {sequence.width}:{sequence.height}</div>{state.selectedMask && <div className="viewer-center-readout"><span className="track-pill"><Brush size={12} /> mask {state.maskDisplay}</span></div>}</div>{(state.workspace === 'mask' || state.workspace === 'maskTracking') && <MaskCanvasToolbar />}<div className="transport"><div className="transport-controls" aria-label="Transport controls"><button className="transport-step" onClick={() => state.stepFrame(-1)} title="Previous frame"><span aria-hidden="true">⏮</span><small>Prev</small></button><button className="transport-play" onClick={state.togglePlay} aria-label={state.isPlaying ? 'Pause' : 'Play'}>{state.isPlaying ? <Pause size={18} fill="currentColor" /> : <Play size={18} fill="currentColor" />}</button><button className="transport-step" onClick={() => state.stepFrame(1)} title="Next frame"><span aria-hidden="true">⏭</span><small>Next</small></button></div><label className="transport-rate"><span>Speed</span><select className="rate-select" value={state.playbackRate} onChange={(event) => state.setPlaybackRate(Number(event.target.value))}><option value="0.25">0.25×</option><option value="0.5">0.5×</option><option value="1">1×</option><option value="2">2×</option></select></label><span className="transport-time">{formatTimelinePosition(state.playhead, activeSequence(state.project).fps, state.timelineDisplay)} <span>/ {formatTimelinePosition(Math.max(0, activeSequence(state.project).duration), activeSequence(state.project).fps, state.timelineDisplay)}</span></span><div className="transport-spacer" /><button className="text-button" onClick={state.addMarker}><Plus size={13} /> marker</button><button className="text-button" onClick={capture}><Camera size={13} /> capture</button></div>{note && <div className="analysis-note">{note}<button onClick={() => setNote('')}><X size={12} /></button></div>}</section>;
}
