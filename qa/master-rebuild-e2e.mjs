...
async function run() {
  ...
  try {
    ...
    // Check Source Monitor is displayed
    const sourceMonitor = page.locator('[data-testid="source-monitor"]')
    assert((await sourceMonitor.count()) > 0, 'Source Monitor is active displaying imported asset')

    // Capture screenshot of Source Monitor
    ...
    // Switch between Source and Program tabs
    const programTab = page.locator('[data-testid="monitor-mode-program-btn"]')
    await programTab.click()
    await page.waitForTimeout(300)
    const monitorMode1 = await page.evaluate(() => window.__omniframe_store.getState().monitorMode)
    assert(monitorMode1 === 'program', 'Monitor switched to Program view')

    const sourceTab = page.locator('[data-testid="monitor-mode-source-btn"]')
    await sourceTab.click()
    await page.waitForTimeout(300)
    const monitorMode2 = await page.evaluate(() => window.__omniframe_store.getState().monitorMode)
    assert(monitorMode2 === 'source', 'Monitor switched to Source view')
