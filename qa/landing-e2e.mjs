...
await page.setViewportSize({width:1440,height:900});await page.getByRole('button',{name:'Open Studio'}).click();await page.waitForTimeout(220);assert(await page.getByTestId('landing-page').evaluate(el=>el.classList.contains('of-landing-leave')),'masked studio transition plays');assert(await page.getByTestId('studio-scene').isVisible(),'studio mounts underneath moving mask');const transitionClip=await page.getByTestId('landing-page').evaluate(el=>getComputedStyle(el).clipPath);assert(transitionClip!=='none'&&!transitionClip.startsWith('inset'),'landing uses a moving polygon mask',transitionClip);await page.screenshot({path:join(SHOTS,'landing-transition.png')});await page.waitForURL('**/#studio',{timeout:2000});assert(page.url().endsWith('/#studio'),'studio route updates hash',page.url());await page.getByText('Your canvas is empty').waitFor({state:'visible'});assert(await page.getByTestId('timeline').isVisible(),'studio remains mounted after transition')
